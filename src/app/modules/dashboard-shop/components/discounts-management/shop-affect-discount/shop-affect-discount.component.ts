...
@Component({
  selector: 'app-shop-affect-discount',
  templateUrl: './shop-affect-discount.component.html',
  styleUrls: ['./shop-affect-discount.component.scss']
})
export class ShopAffectDiscountComponent implements OnInit {
  date;
  ...
  getFormValues(discount, val, startdate, enddate) {
    ...
    return this.discountService.addPromotionS2c(el).subscribe(
      res => {
        // this.redirectTo('/shop/offline-discounts-list');
        this.router.navigate(['/shop/offline-discounts-list']);

        this.snackbar.open(this.translate.currentLang === 'Chinese' ? "折扣信息已成功更改":this.translate.currentLang === 'Italian' ? 'Sconto modificato con successo':'Discount added successfully', '',{
            duration: 1000
          });
      }, err => {
        console.log(err);
