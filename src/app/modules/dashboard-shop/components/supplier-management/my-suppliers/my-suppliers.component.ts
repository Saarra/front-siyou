...
export class MySuppliersComponent implements OnInit {
  ...
  onGetSuppliersList(){
    ...
    this.suppcontact=0;
    this.suppemail=0;
    return this.supplierService.GetAddedSuppliers().subscribe(
      res=>{
        this.suppliersList= res;
        this.suppliersData=this.suppliersList.data
        this.dataSource.data =this.suppliersData;
        this.suppliersNum=this.suppliersData.length
 ...
 onDeleteSupplier(id){
  ...
  return this.supplierService.DeleteMySupplier(id).subscribe(
    res=>{
